...
def _final_validate(config):
    ...
    # Validate the processor bound to this stack, not another AFE instance.
    afe_configs = full_config.get("esp_afe", [])
    if isinstance(afe_configs, dict):
        afe_configs = [afe_configs]
    processor_id = config.get(CONF_PROCESSOR_ID)
    afe_conf = next(
        (afe for afe in afe_configs
         if processor_id is not None and afe[CONF_ID] == processor_id),
        {},
    )
    afe_mics = afe_conf.get("mic_num", 1)
    if afe_mics >= 2:
        tdm_dual = (
            config.get(CONF_USE_TDM_REFERENCE, False) or CONF_TDM_MIC_SLOTS in config
...
async def to_code(config):
    ...
    if CONF_RX_MIC_SLOTS in config:
        cg.add_define("USE_ESP_AUDIO_STACK_STD_DUAL_MIC")
