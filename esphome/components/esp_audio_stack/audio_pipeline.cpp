...
bool ESPAudioStack::allocate_audio_buffers_(AudioTaskCtx &ctx) {
  ...
  bool dual_mic_configured = this->tdm_second_mic_slot_ >= 0;
#ifdef USE_ESP_AUDIO_STACK_STD_DUAL_MIC
  dual_mic_configured = dual_mic_configured || this->std_second_mic_slot_ >= 0;
#endif
  const uint8_t worst_mic_ch = dual_mic_configured ? 2 : 1;
...
#else
  const bool have_std_dual_mic = false;
#endif
