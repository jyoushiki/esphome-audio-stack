...
class EspAfe final : public Component, public AudioProcessor {
 ...
 protected:
...
#ifdef USE_ESP_AFE_POST_AGC
  // Espressif AGC consumes 10ms at 16kHz. One quantum is assembled while the
  // previous one is emitted, independent of consumer-frame fragmentation.
  ...
  // GMF creates its wake/VAD state mutex only while opening an element whose
  // VAD stage is enabled. Defer an initial requested OFF state until the first
  // output callback, which necessarily runs after open has completed.
