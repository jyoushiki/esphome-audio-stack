...
```text
[mic1 + mic2 + ref] -> |AEC| -> |Speech Enhancement/BSS| -> |post-AFE AGC| -> [clean output]
```
...
> adds a fixed 10 ms causal delay. AGC changes still require AFE reinit.
...
| `agc_enabled` | bool | **true** | Enable automatic gain control. Uses ESP-SR's WebRTC stage for one mic and an explicit post-AFE WebRTC stage for two mics |
...
| `agc_compression_gain` | int | `9` | AGC compression gain in dB (0-30), also used by the dual-mic post-AFE stage |
| `agc_target_level` | int | `3` | AGC target level (0-31, lower value = louder output), also used by the dual-mic post-AFE stage |
...
represents NS/AGC changes as config changes and rebuilds the AFE instance. On
dual-mic builds the rebuild also creates or destroys the post-AFE WebRTC AGC.
...
`post_afe_agc_support: auto` (default) compiles the post-AFE AGC only for
dual-microphone configurations with `agc_enabled: true` or an AGC switch.
Set it to `true` when a lambda will enable AGC later. Set it to `false` to
exclude it explicitly; validation rejects an enabled dual-mic AGC or its switch
in that case. Single-microphone AGC still uses ESP-SR directly.

The adapter uses two 160-sample blocks (640 bytes of PCM storage, plus the
opaque Espressif AGC state), with a fixed 10 ms delay. It neither allocates a
consumer-frame FIFO nor moves queued frames. An AGC processing error bypasses
only the affected 10 ms block and preserves timing. Firmware without this
feature excludes its PCM storage and processing code.
