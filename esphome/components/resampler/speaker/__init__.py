...
from esphome.components import audio, esp32, psram, speaker
...
def _validate_task_core(config):
    if config.get(CONF_TASK_CORE) == 1 and esp32.get_esp32_variant() not in (
        esp32.VARIANT_ESP32, esp32.VARIANT_ESP32S3,
        esp32.VARIANT_ESP32S31, esp32.VARIANT_ESP32P4,
    ):
        raise cv.Invalid("task_core: 1 requires a supported dual-core ESP32")
    return config
